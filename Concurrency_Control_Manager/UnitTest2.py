...
ConcurrencyControlManager = classes.ConcurrencyControlManager
...
class TestConcurrencyControlManager(unittest.TestCase):
    ...
    def test_strategy_switching(self):
        ...
        # Switch to lock-based first to ensure we start from a known state
        self.ccm.set_concurrency_mechanism("lock-based")
        ...
        self.ccm.set_concurrency_mechanism("validation-based")
        self.assertEqual(
            self.ccm.strategy.__class__.__name__, "ValidationBasedStrategy"
        )
...
if __name__ == "__main__":
    ...
    suite_ts = unittest.TestLoader().loadTestsFromTestCase(
        TestTimestampBasedConcurrency
    )
    unittest.TextTestRunner(verbosity=2).run(suite_ts)

    print("\n--- Test 3: Validation-Based Strategy (OCC) ---")
    suite_occ = unittest.TestLoader().loadTestsFromTestCase(
        TestValidationBasedConcurrency
    )
    unittest.TextTestRunner(verbosity=2).run(suite_occ)

    print("\n--- Test 4: Multi-Version Strategy (MVCC) ---")
    suite_mvcc = unittest.TestLoader().loadTestsFromTestCase(
        TestMultiVersionConcurrency
    )
    ...
    suite_ccm = unittest.TestLoader().loadTestsFromTestCase(
        TestConcurrencyControlManager
    )
