...
class ClientExecutionLog:
    ...
    def __repr__(self):
        ...
        if self.error_message:
            return (
                f"[{time_str}] {status} {self.operation} - ERROR: {self.error_message}"
            )
        else:
...
class Client:
    ...
    def __init__(self, ccm, client_name: Optional[str] = None):
        ...
        self.verbose = False  # Disabled by default for cleaner output

    def begin_transaction(self) -> int:
        """Start a new transaction."""
        if self.current_transaction_id is not None:
            raise Exception(
                f"{self.client_name} already has an active transaction {self.current_transaction_id}"
            )
    ...
    def read(self, object_id: str):
        ...
        try:
            ...
            self._log_operation(
                operation, success=True, transaction_id=self.current_transaction_id
            )
    ...
    def write(self, object_id: str, value: Any = None):
        ...
        operation = ClientOperation(
            operation_type="write", object_id=object_id, value=value
        )
    ...
    def commit(self):
        ...
        try:
            ...
            self._log_operation(
                operation, success=False, error_message=str(e), transaction_id=tx_id
            )
            self.current_transaction_id = None
            raise
    ...
    def execute_script(
        self, operations: List[ClientOperation], auto_commit: bool = True
    ):
    ...
    def _ensure_active_transaction(self):
        ...
        if self.current_transaction_id is None:
            raise Exception(
                f"{self.client_name} has no active transaction. Call begin_transaction() first."
            )
