...
class BufferManager:
    ...
    def __init__(self, actual_storage_engine: Any = MockStorageEngine, capacity: int = BUFFER_CAPACITY):
    ...
    def write_block(self, table_name: str, pk_value: Any, new_data: dict) -> MockChangeReport:
        key = self._get_buffer_key(table_name, pk_value)
        ...
        row.data = new_data
        row.is_dirty = True
        print(f"[Buffer] Data {pk_value} diubah dan ditandai kotor.")
        return MockChangeReport(table_name, old_data, new_data) # Mengembalikan laporan perubahan untuk LogWriter
