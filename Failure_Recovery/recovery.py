...
class RecoveryEngine:
    ...
    def recover(self) -> Dict[str, Any]:
        ...
        if checkpoint_found:
            ...
            print(
                f"[Recovery] Found checkpoint with {len(undo_list)} ongoing transactions: {undo_list}")
        else:
            undo_list = set()
        ...
        for entry in entries_after:
            ...
            if action in ["insert", "update", "delete"]:
                print(
                    f"[Recovery] REDO: TX {tx_id} {action.upper()} on {entry.table_name}")
        ...
        for entry in self.wal_manager.iter_backward():
            ...
            if tx_id in undo_list and action in ["insert", "update", "delete"]:
                print(
                    f"[Recovery] Undoing TX {tx_id} {action.upper()} on {entry.table_name}")
    ...
    def abort_transaction(self, tx_id: int) -> None:
        ...
        for entry in self.wal_manager.iter_backward(criteria):
            entry_count += 1
            print(
                f"[Recovery Debug] Entry {entry_count}: action={entry.action}, tx_id={entry.transaction_id}, matches={entry.transaction_id == tx_id}")
            ...
            if entry.action == "start":
                print(
                    f"[Recovery Debug]   -> Reached START for TX {tx_id}, stopping search")
                break

            elif entry.action in ["insert", "update", "delete"]:
                print(
                    f"[Recovery Debug]   -> Found operation to undo: {entry.action} on {entry.table_name}")
                undo_operations.append(entry)

        print(f"[Recovery Debug] Iterated {entry_count} entries total")
        print(
            f"[Recovery Debug] Collected {len(undo_operations)} operations to undo")

        for entry in undo_operations:
            print(
                f"[Recovery] Undoing TX {tx_id} {entry.action.upper()} on {entry.table_name}")
            self._apply_undo(entry)
            self._write_compensation_log(entry)
    ...
    def _write_compensation_log(self, entry):
        ...
        compensation_entry = {
            ...
            # What was there before undo (for documentation)
        ...
        print(
            f"    [CLR] Compensation log written for TX {entry.transaction_id}")
    ...
    def _apply_redo(self, entry: LogEntry) -> None:
        ...
        try:
            if action == "insert" and new_data:
                self.buffer_manager.write_to_buffer_for_recovery(
                    table, pk, new_data)
