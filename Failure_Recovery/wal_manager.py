...
class WALManager:
    ...
    def log_insert(self, transaction_id: Union[int, str], table_name: str,
        ...
        tx_id = int(transaction_id) if isinstance(transaction_id, str) else transaction_id
    ...
    def log_checkpoint(self, ongoing_transactions: List[Union[int, str]]):
        ...
        tx_ids = [int(tx) if isinstance(tx, str) else tx for tx in ongoing_transactions]
        ...
        self._write_entry(entry)
        print(f"[WAL] Checkpoint logged with {len(tx_ids)} ongoing transactions")
    ...
    def clear_wal_before_oldest_transaction(self, ongoing_transactions: List[Union[int, str]]):
        ...
        ongoing_tx_set = set(int(tx) if isinstance(tx, str) else tx for tx in ongoing_transactions)
    ...
    def clear(self, keep_from_transaction: Optional[Union[int, str]] = None):
        ...
        keep_tx = int(keep_from_transaction) if isinstance(keep_from_transaction, str) else keep_from_transaction
        ...
        cleared_count = len(lines) - len(entries_to_keep)
        print(f"[WAL] Cleared {cleared_count} entries, kept {len(entries_to_keep)}")
